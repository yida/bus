...
%  plot(ItsCorners(:,1), ItsCorners(:,2), '*');
  ...
  for i = 1 : 4
    ...
    plot_circle(circle);
    plot([ItsCorners(i,1), ItsCorners(i,1)],...
         [ItsCorners(i,2)+CornetR*signMat2(i,2), ItsCorners(i,2)+CurbExt(i,1)]);
    plot([ItsCorners(i,1)+CornetR*signMat2(i,1), ItsCorners(i,1)+CurbExt(i,2)],...
  ...
  for i = 1 : 4 
    plot([(CirCorners(i,1)+CirCorners(i+1,1))/2 + CornetR*dirSign(i,1),...
