...
require 'torch'
require 'unix'
package.cpath = '/home/yida/UPennTHOR/Tools/lua_msgpack/?.so;'..package.cpath
require 'msgpack'
...
local dataset = loadDataMP(datasetpath, 'statemp-04.09.2013.00.57.15-0', _, 1)
print(#dataset)
...
--  if dataset[i].timestamp >= 946687159.94 and dataset[i].timestamp <= 946687184.94 then 
    if dataset[i].type == 'state' then
      local str = msgpack.pack(dataset[i]); 
      print(#str)
