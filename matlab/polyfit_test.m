...
%        [start_span, end_span] = range2index(left_fitted_x, -quadratic.span_left_ts/2, quadratic.span_left_ts/2);
%        plot(left_fitted_x(start_span:end_span), left_fitted_y(start_span:end_span), 'r.');

      centralized_ts = centralize(sample{sample_counter}.ts, sample{sample_counter}.wy);
      Y_left = polyval(quadratic.left_p, centralized_ts);
      Q_left = sum((sample{sample_counter}.wy - Y_left).^2);
      Y_right = polyval(quadratic.right_p, centralized_ts);
...
while imu_idx < imu_sample_num & offset_idx < imu_sample_num
  ...
  while (abs(imu_ts_filter(offset_idx) - imu_ts_filter(imu_idx)) < quadratic.span_right_ts) & (offset_idx < imu_sample_num) 
...
while imu_idx < imu_sample_num & offset_idx < imu_sample_num
  offset_idx = imu_idx + 1;
  while (abs(imu_ts_filter(offset_idx) - imu_ts_filter(imu_idx)) < quadratic.span_left_ts) & (offset_idx < imu_sample_num) 
    offset_idx = offset_idx + 1;
  end
%  fprintf(1, '%d %d %d\n', imu_idx, offset_idx, offset_idx - imu_idx);
  mid_idx = floor((imu_idx + offset_idx)/2);
